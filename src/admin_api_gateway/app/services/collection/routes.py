...
def get_items(admin_id):
    response = requests.get(URL + f'/admin/{admin_id}/collection', headers=request.headers)
...
def get_item_by_id(admin_id, item_id):
    response = requests.get(f'{URL}/admin/{admin_id}/item/{item_id}', headers=request.headers)
    return response.json(), response.status_code
...
def update_item(admin_id, item_id):
    response = requests.post(f'{URL}/admin/{admin_id}/item/{item_id}', json=request.get_json(), headers=request.headers)
    return response.json(), response.status_code
...
def add_item(admin_id):
    response = requests.put(f'{URL}/admin/{admin_id}/item/', json=request.get_json(), headers=request.headers)
    return response.json(), response.status_code
