...
@account_bp.route("/user/<int:user_id>", methods=["POST"])
def change_user_info(user_id):
    response = HttpClient.post(
        URL + f"/user/{user_id}", headers=request.headers, json=request.get_json()
    )
    return response.json(), response.status_code
...
def get_user_by_id(user_id):
    response = HttpClient.get(URL + f"/user/{user_id}", headers=request.headers)
    return response.json(), response.status_code
